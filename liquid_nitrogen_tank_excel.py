...
EXCEL_EN = {
    ...
    '盒位编号由“列 + 层”直接拼接；列使用1–9，层使用1–20。': 'A box code concatenates column (1-9) and level (1-20).',
    ...
    '液氮罐默认4列×5层；每个液氮罐可独立调整为1–9列×1–20层。': 'Tanks default to 4 columns x 5 levels. Each tank can have 1-9 columns and 1-20 levels.',
...
def _parse_unit_code(value: str) -> tuple[int, int] | None:
    match = re.fullmatch(r"([1-9])([1-9]|1[0-9]|20)", value)
    if not match:
        return None
...
def _ledger_sheet(
    ...
    coordinates = [
        parsed
        for code in records
        if (parsed := _parse_unit_code(str(code))) is not None
    ]
...
def _instructions_sheet(freezer_count: int, language: str = 'zh') -> str:
    contents = (
        ("使用说明", "本文件由液氮罐管理程序生成。"),
        ("编号规则", "盒位编号由“列 + 层”直接拼接；列使用1–9，层使用1–20。"),
...
def _tube_instructions_sheet(freezer_count: int, language: str = 'zh') -> str:
    contents = [
        ...
        ("规格规则", "液氮罐默认4列×5层；每个液氮罐可独立调整为1–9列×1–20层。"),
            ...
            for freezer_index, (sheet_name, sheet_path) in enumerate(ledger_entries):
                ...
                if tube_format:
                    ...
                    for row_number, values in enumerate(rows[1:], 2):
                        ...
                        if not box_code and not position and not has_sample_data:
                            continue
                        if _parse_unit_code(box_code) is None:
                            issues.append(
                                ImportIssue(
                                    "错误",
                                    sheet_name,
                ...
                for row_number, values in enumerate(rows[1:], 2):
                    ...
                    if not code and not sample_name and not experiment and not has_other_data:
                        continue
                    if _parse_unit_code(code) is None:
                        issues.append(ImportIssue("错误", sheet_name, row_number, "盒位编号", code, "盒位编码必须是有效编码，例如 41 或 410。"))
                        continue
